...
Но здесь висит вопрос поглубже: если модель продолжит становиться сильнее, не будут ли сегодняшние Harness в итоге «съедены» моделью? Ричард Саттон в «Горьком уроке» (The Bitter Lesson) вспоминает картину, повторявшуюся снова и снова за семьдесят лет исследований ИИ[^ch1-1]: раз за разом исследователи кодируют своё понимание предметной области в систему, что даёт эффект в краткосрочной перспективе, но в долгосрочной всегда проигрывает универсальным методам, способным масштабироваться вместе с вычислительными мощностями и объёмом данных, — поиску и обучению. Если мерить этой меркой, то сколько из ограничений, проверки и исправления внутри Harness относится к «человеческим априорным знаниям» и обречено быть внутренне усвоено моделью? Позиция этой книги укладывается в восемь слов: **согласны с направлением, прагматичны в темпе**. По направлению книга не сомневается, что модель будет и дальше поглощать Harness: и вызов инструментов, и долгосрочное планирование когда-то держались на внешней оркестровке, а теперь стали врождёнными способностями модели. Но по темпу это «поглощение» происходит куда медленнее, чем подсказывает интуиция: обучение измеряется месяцами, и модель не способна за один раз усвоить все ограничения и предпочтения реального бизнеса; граница возможностей модели в данный момент — это и есть ценность Harness в данный момент. Поэтому Harness-инженерия — не сопротивление горькому уроку, а практическое воплощение этого урока в масштабе инженерного времени: то, что модель ещё делает нестабильно, Harness подхватывает первым; с каждым слоем, который модель усваивает внутренне, Harness сбрасывает один слой и вместо этого страхует новый рубеж возможностей. Эта основная линия пройдёт через всю книгу — вторая глава даёт прагматичный ответ с точки зрения инженерии контекста, восьмая глава обсуждает, как агент сам может открывать структуру знаний и способностей, а послесловие вернётся к полному ответу на вопрос «съест ли модель Harness».
...
Выше мы обсудили, как модель через обучение с подкреплением превращает стратегию принятия решений о вызове инструментов во врождённую способность. Но обучение агента происходит не только на этапе тренировки: некоторые читатели, услышав об обучении агента на опыте, сразу думают, что для этого обязательно нужно тренировать модель. На самом деле постобучение — не единственный способ, которым агент учится на опыте. Механизмы обучения агента можно свести к трём взаимодополняющим парадигмам (рис. 1-1):

![Рис. 1-1 Три парадигмы обучения агента](images/fig1-1.svg)

- **Постобучение (Post-training)**: через обучение с подкреплением опыт закрепляется в параметрах модели, что даёт максимальную кросс-задачную универсальность, но обновление обходится дорого (подробнее — в седьмой главе).
- **Обучение в контексте (In-Context Learning)**: быстрая адаптация по принципу поиска шаблонов в контексте с помощью механизма внимания (Attention Mechanism — механизм, которым модель при обработке входных данных решает, «на какую информацию обратить внимание»). Например, если в промпте показать модели несколько примеров обработки диалогов со службой поддержки (вроде «жалоба пользователя → успокоить + предложить компенсацию»), она сможет обработать новый диалог аналогичным образом — это и есть обучение в контексте. Такое обучение позволяет быстро адаптироваться, но носит временный характер: с окончанием сессии оно исчезает. Стоит пояснить: хотя оно и называется «обучением», его внутренний механизм ближе к **сопоставлению шаблонов, чем к настоящему обучению**. Аналогия: если показать вам три однотипные математические задачи с ответами, а затем дать четвёртую, вы, скорее всего, сумеете решить её по образцу — вот что делает обучение в контексте. Но если четвёртая задача требует совершенно нового подхода к решению, одного взгляда на ответы к первым трём недостаточно. Иными словами, обучение в контексте позволяет модели **применять уже виденные шаблоны**, но не позволяет **открывать совершенно новые закономерности** — и это принципиально отличает его от постобучения (во второй главе этот тезис будет подробно раскрыт с точки зрения механизма внимания).
- **Экстернализованное обучение (Externalized Learning)**: вынесение знаний и процессов наружу — в базы знаний и исполняемый код инструментов, что сочетает в себе долговечность и объяснимость.

Эти три парадигмы дополняют друг друга на разных временных масштабах: постобучение даёт базовые способности, обучение в контексте обеспечивает быструю адаптацию, а экстернализованное обучение гарантирует надёжность и эффективность. Восьмая глава систематически сравнит взаимосвязи между этими тремя парадигмами.

Аналогия: постобучение похоже на системное изучение учебника — после него способности возрастают навсегда, но учиться дорого; обучение в контексте — как заглядывание в справочник на месте: пока есть материал, справишься, закрыл — забыл; экстернализованное обучение — как ведение личного блокнота: информация хранится долго и доступна в любой момент, но её нужно специально систематизировать.
...
| Проверка и исправление инструментов | Глава 5 (генерация кода) | Harness кодинг-агента, разработка через тесты, правила в виде кода | Подмена личности, распределение ответственности |
...
| Исправление на системном уровне | Глава 8 (самоэволюция) | Экстернализованное обучение, создание инструментов, накопление опыта | — |
