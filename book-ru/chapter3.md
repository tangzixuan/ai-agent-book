...
В завершение стоит провести чёткую границу, чтобы не путать этот раздел с другими главами книги: здесь речь идёт об алгоритмах упорядочивания на **уровне хранения памяти** — какие записи отбирать, кластеризовать, в какую форму абстрагировать. Сжатие контекста из второй главы решает проблему окна в рамках одной сессии — это уровень другого порядка. А то, как эти алгоритмы упорядочивания запускаются в продакшн-системах — механизм триггеров периодической, асинхронной офлайн-консолидации памяти и её инженерная реализация, — будет раскрыто в восьмой главе.
...
Выбор чистого текста Markdown вместо специализированной базы данных в качестве базового представления знаний — решение, на первый взгляд контринтуитивное, но глубоко продуманное с инженерной точки зрения (в пятой главе будет подробно рассмотрен аналогичный выбор OpenClaw — открытого фреймворка агентов). Чистый текст означает, что пользователь может напрямую читать, редактировать и исправлять знания агента; можно использовать контроль версий и откат через Git; и что важнее — обладая способностью `write_file`, агент может самостоятельно записывать и организовывать знания. По завершении сессии система автоматически анализирует диалог, записывает обновления пользовательских предпочтений в `user/memories/`, а операционный опыт — в `agent/memories/`, формируя цикл самоэволюции памяти — это и есть инженерная реализация парадигмы «экстернализованного обучения», которую подробно обсудим в восьмой главе.
...
Эта глава и предыдущая обе имеют дело с проблемой «контекста» — одна в рамках одной сессии, другая через множество сессий. Следующая глава переходит к теме «инструментов»: как агент взаимодействует с внешним миром через инструменты, включая проектирование инструментов, стандарт взаимодействия MCP и архитектуру, управляемую событиями.
