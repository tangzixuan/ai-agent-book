...
Поскольку агенты не разделяют контекст, информацию приходится передавать через явный механизм коммуникации. Наиболее распространены три способа:

- **Параметры вызова инструмента**: вышестоящий агент передаёт структурированные данные как параметры инструменту нижестоящего агента — подходит для сценариев, где нужны чёткая типизация и понятная структура;
...
| Reviewer проверяет код по результатам выполнения тестов | Да (обратная связь от выполнения) | Значительное улучшение |
| Reviewer проверяет фронтенд-код/код PPT по скриншотам рендеринга | Да (визуальная обратная связь) | Значительное улучшение |
| Reviewer проверяет факты с помощью внешних инструментов | Да (обратная связь от инструмента) | Значительное улучшение |
...
В мультиагентном взаимодействии с общим контекстом каждый этап представляет собой отдельного агента (со своим системным промптом и набором инструментов), но он наследует полную траекторию предыдущего агента — как коллега, принимающий смену, может пролистать все рабочие журналы предшественника. Ключевое преимущество такого «наследуемого сотрудничества» — нулевая потеря информации, каждый агент может обратиться к деталям любого предыдущего этапа. Сложность же в том, чтобы текущий агент сосредоточился на своих основных обязанностях, не отвлекаясь на большой объём унаследованной исторической информации.

### Многоэтапная смена ролей

Сначала проясним один терминологический спор: если пользоваться языком первой главы, многоэтапная смена ролей — это разновидность **оркестрации в стиле рабочего процесса**: маршрут исполнения (например, уточнение требований → реализация → проверка) задан заранее. Причина, по которой эта глава пересматривает её в рамках мультиагентности, — точка зрения на идентичность агента и контекст: когда системный промпт, набор инструментов и фокус внимания на каждом этапе различаются, полезно рассматривать эти этапы как нескольких агентов, разделяющих одну и ту же траекторию, — это даёт реальную выгоду в проектировании: промпт и набор инструментов для каждой «идентичности» можно шлифовать независимо, а границы этапов естественным образом становятся точками контроля качества.

В сложных задачах роль и обязанности агента могут заметно меняться на разных этапах. Если всё время использовать один и тот же статический системный промпт, он либо получается слишком общим и без конкретики, либо в него приходится запихивать указания сразу для всех этапов, отчего он становится чрезмерно длинным. Подход многоэтапной смены ролей заключается в следующем: в зависимости от текущего этапа динамически переключать системный промпт и набор инструментов, чтобы агент на каждом этапе работал в наиболее подходящей для него «идентичности». Такое переключение не требует создания нового экземпляра или запуска нового процесса — обновляется только контекст в рамках одной и той же сессии выполнения. Ключевой момент в том, что, хотя роль сменилась, история диалога и состояние задачи остаются непрерывными и общими — в новой роли агент по-прежнему имеет доступ ко всей информации, накопленной на предыдущих этапах.


![Рис. 10-2 Смена ролей в зависимости от этапа](images/fig10-2.svg)


> **Эксперимент 10-1 ★★: определение системного промпта в зависимости от этапа выполнения**
>
> Этот эксперимент на примере полного рабочего процесса кодинг-агента демонстрирует, как поэтапный системный промпт улучшает работу агента.
>
> **Сценарий задачи**: пользователь формулирует требование к разработке ПО, агент последовательно проходит через три этапа: уточнение требований, реализация кода, контроль качества.
>
> **Этап первый: уточнение требований** (роль: аналитик требований)
>
> Системный промпт подчёркивает:
> - «Твоя обязанность — полностью понять требования пользователя. Задавай вопросы, чтобы прояснить неясные места, убедись, что ты до конца понимаешь ожидаемую функциональность, сценарии использования и требования к производительности».
> - «Не спеши с реализацией. На этом этапе твоя задача — задавать вопросы и получать подтверждения, а не писать код».
> - «Когда убедишься, что все ключевые требования прояснены, вызови инструмент `complete_requirements_analysis()`, чтобы завершить этот этап».
>
> Набор инструментов ограничен: `ask_clarifying_question(question)` используется, чтобы задавать пользователю уточняющие вопросы, `save_requirement(key, value)` — чтобы фиксировать подтверждённые пункты требований, `complete_requirements_analysis()` — чтобы отметить завершение этапа.
>
> Агент ведёт с пользователем многотуровый диалог: «Файлы каких типов должен обрабатывать этот скрипт?», «Нужно ли рекурсивно обрабатывать подпапки?», «Сохранять ли исходные имена файлов после перемещения?». Через эти вопросы агент постепенно выстраивает полное понимание требований и сохраняет их в структурированном виде. Когда агент решает, что требования достаточно ясны, он вызывает `complete_requirements_analysis()`, запуская смену роли — система обнаруживает сигнал о завершении этапа и автоматически переключается на конфигурацию следующего этапа.
>
> **Этап второй: реализация кода** (роль: инженер-программист)
>
> Новый системный промпт подчёркивает:
> - «Твоя обязанность — написать качественный код на Python на основе подтверждённых требований».
> - «Следуй лучшим практикам: код должен быть модульным, содержать корректную обработку ошибок и необходимые комментарии».
> - «После того как код написан и прошёл базовые тесты, вызови `submit_for_review()`, чтобы перейти к этапу проверки».
>
> Набор инструментов заметно изменился: инструменты для уточнения требований убраны, вместо них появились инструменты разработки `write_file(path, content)`, `read_file(path)`, `execute_code(code)` и т. д. Агент начинает писать код на основе требований, сохранённых на первом этапе, — сначала основная логика, затем добавление обработки ошибок, наконец написание тестов для проверки. На протяжении всего процесса агент по-прежнему может обращаться к истории диалога первого этапа, чтобы вспомнить детали требований, но модель поведения уже совершенно иная: больше нет вопросов, всё внимание сосредоточено на реализации. По завершении вызывается `submit_for_review()`.
>
> **Этап третий: проверка кода** (роль: рецензент кода)
>
> Новый системный промпт подчёркивает:
> - «Твоя обязанность — проверить только что написанный код и оценить его качество по нескольким измерениям: корректность функциональности, соблюдение стандартов кода, обработка ошибок, оптимизация производительности, безопасность».
> - «Применяй критическое мышление, старайся найти возможные проблемы и точки для улучшения в коде».
> - «Если обнаружены серьёзные проблемы, вызови `request_revision(issues)`, чтобы вернуться на этап реализации для исправлений; если качество приемлемое, вызови `approve_code()`, чтобы завершить задачу».
>
> Набор инструментов снова меняется: теперь это инструменты анализа качества кода `run_linter(file)`, `run_tests(file)`, `analyze_complexity(file)` и т. д. Агент пересматривает код с точки зрения рецензента, запускает статический анализ, ищет потенциальные баги, проблемы производительности или уязвимости безопасности.
>
> Такая трёхэтапная конструкция позволяет агенту на каждом этапе сосредоточиться на текущей ключевой задаче. Что ещё важнее, чёткий механизм перехода между этапами гарантирует целостность выполнения задачи — агент не перескочит через анализ требований прямо к написанию кода и не сдаст результат без проверки.
>
> **Требования к эксперименту**:
> 1. Реализовать трёхэтапные системные промпты, с чётким определением роли и указаниями по поведению для каждого этапа
> 2. Настроить для каждого этапа соответствующий набор инструментов
> 3. Реализовать механизм запуска перехода между этапами (через вызов определённого инструмента)
> 4. Обеспечить непрерывность контекста между этапами
> 5. Обработать случай отката — при обнаружении проблем на проверке кода нужно уметь вернуться на этап реализации
> 6. Вести журнал выполнения каждого этапа, показывающий, как разные промпты порождают разные модели поведения
>

### Смена ролей между предметными областями

Многоэтапная смена ролей, рассмотренная выше, демонстрирует поэтапное выполнение в рамках одного типа задачи (разработка ПО). Смена ролей между предметными областями идёт дальше и исследует, как агент автономно переключается между разными типами задач — это уже не заранее спланированный линейный процесс, а самостоятельное решение агента о том, на какую специализированную роль переключиться, в зависимости от изменения запроса пользователя.

> **Эксперимент 10-2 ★★: смена нескольких ролей**
>
> **Предварительные требования**: рекомендуется сначала ознакомиться с механизмом Agent Skills из второй главы.
>
> **Архитектура системы**: пять ролей —
>
> - **triage (диспетчер на входе, роль по умолчанию)**: понимает общий запрос пользователя, разбивает его на упорядоченные подзадачи, поочерёдно передаёт их подходящим специализированным ролям и подводит итог после завершения всех подзадач. Собственных специализированных инструментов не имеет, располагает только transfer
> - **research (эксперт по поиску информации)**: с помощью `web_search` ищет данные, факты и материалы
> - **coding (эксперт по программированию)**: с помощью `execute_python` пишет и запускает код, решает задачи программной логики/скриптов
> - **data_analysis (эксперт по анализу данных)**: с помощью `calculate` / `descriptive_stats` выполняет количественные расчёты и статистику (например, темп прироста год к году, среднегодовой темп роста CAGR, среднее значение)
> - **writing (эксперт по написанию текстов)**: превращает найденные данные и выводы расчётов в связный, ориентированный на указанную аудиторию текст (может использовать `count_characters` для грубой оценки объёма)
>
> **Ключевой механизм: инструмент transfer_to_agent**
>
> Все роли оснащены инструментом `transfer_to_agent(target_role, reason)`. При его вызове система последовательно: 1) сохраняет текущую историю диалога; 2) загружает промпт и набор инструментов целевой роли; 3) передаёт историю диалога новой роли, чтобы та поняла контекст; 4) продолжает выполнение уже от лица новой роли.
>
> **Сценарий эксперимента**: по умолчанию система работает от лица triage (диспетчера на входе). Пользователь ставит комплексную задачу на стыке нескольких областей: «Я готовлю материалы для инвесторов, помоги мне узнать объёмы продаж новых энергетических автомобилей в Китае за 2021, 2022, 2023 годы, посчитать среднегодовой темп роста (CAGR) за эти три года и написать резюме на китайском языке объёмом не более 120 знаков, ориентированное на инвесторов». triage разбивает это на «найти данные → рассчитать показатели → написать текст» и на первом шаге передаёт управление поиску:
>
> ```python
> transfer_to_agent(target_role="research", reason="сначала нужно найти данные о продажах за три года")
> ```
>
> research находит данные о продажах с помощью `web_search`, записывает ключевые данные в диалог, затем передаёт управление анализу данных:
>
> ```python
> transfer_to_agent(target_role="data_analysis", reason="данные готовы, нужно рассчитать CAGR за три года")
> ```
>
> data_analysis с помощью `calculate` рассчитывает темп роста и передаёт управление writing для составления текста; writing пишет текст и передаёт управление обратно triage для подведения итога. Вся цепочка выглядит так: triage → research → data_analysis → writing → triage, и каждая роль видит полную историю диалога, поэтому следующая роль естественным образом знает, что уже сделано предыдущей.
>
> Решение о смене роли опирается на указания системного промпта. В промпте triage чётко перечислены правила маршрутизации: поиск данных/материалов — передать research, написание и запуск кода — передать coding, количественные расчёты и статистика — передать data_analysis, доработка текста — передать writing. Критерий прост: если задача требует глубоких знаний конкретной предметной области или специализированных инструментов, она передаётся соответствующей специализированной роли. В промптах специализированных ролей также указано, кому передать управление после завершения своей части — конкретной роли или обратно triage.
>
> **Требования к эксперименту**:
> 1. Реализовать системные промпты и специализированные наборы инструментов как минимум для трёх специализированных ролей
...
> 4. Обработать проблему циклического переключения — не допускать, чтобы агент многократно переключался между ролями туда-обратно
> 5. Спроектировать сложный процесс задачи, охватывающий несколько предметных областей, чтобы продемонстрировать ценность смены ролей
>
...
Файловая система решает проблему **обмена артефактами** между агентами, но для сотрудничества нужна ещё и **плоскость управления**: она должна поддерживать передачу сообщений между агентами, запрос состояния и прерывание выполнения. В четвёртой главе уже даны инструментальные примитивы этой плоскости — создание (`spawn_subagent`), отправка сообщения (`send_message_to_subagent`), отмена (`cancel_subagent`) — а также четыре формы взаимодействия: синхронная, асинхронная, потоковая и многораундовая. В этом разделе мы не будем повторять описание интерфейсов, а сосредоточимся на трёх возможностях, от которых зависит мультиагентное взаимодействие, но которые часто упускают из виду.

**Первое. Передача сообщений.** Простейшая форма — точка-точка: агент A напрямую вызывает `send_message_to_agent_b(content)`, что подходит для сценариев с фиксированной топологией и небольшим числом агентов (например, для пары «телефон + компьютер» из эксперимента 10-4 этой главы). Когда число агентов растёт и требуется асинхронная параллельная работа, число прямых соединений растёт квадратично от числа агентов, а кроме того требуется, чтобы отправитель и получатель были онлайн одновременно; в этом случае стоит перейти на **шину сообщений** (подробнее — в разделе «Формы параллельной координации» далее в этой главе): агент публикует сообщение в шину, а та рассылает его по подпискам, так что отправителю не нужно знать своих потребителей. Независимо от того, идёт ли передача напрямую или через шину, сообщение обычно должно нести структурированный **конверт** (envelope): ID отправителя, назначение (конкретный агент или широковещательная рассылка), тип сообщения (например, `task_assigned`/`status_update`/`result`/`terminate`) и полезную нагрузку в формате JSON. Единый формат конверта гарантирует, что получатель надёжно маршрутизирует и разбирает сообщение, а также делает цепочку взаимодействия отслеживаемой — это ключевой момент для отладки многоагентных систем.

**Второе. Запрос состояния.** Это самое недооценённое звено плоскости управления. Если главный агент, отправив дочернего агента на задачу, не может узнать о его прогрессе, то он не в состоянии ни решить, стоит ли продолжать ждать, ни своевременно вмешаться, если тот застрял. Существует две парадигмы получения состояния. **Опрос (pull)**: главный агент вызывает `get_subagent_status(agent_id)`, получая текущее состояние дочернего агента (выполняется/ждёт ввода/завершено/провалено), прогресс и время последней активности. **Проталкивание (push)**: дочерний агент сам публикует обновления состояния в шину сообщений в ходе выполнения, а главный агент поддерживает таблицу состояний задач, обновляемую в реальном времени (именно такая схема реализована в «мониторинге в реальном времени» из эксперимента 10-6 этой главы). У каждого подхода свои компромиссы: опрос прост в реализации, но при слишком частых запросах впустую тратятся токены, а при слишком редких — теряется своевременность; проталкивание даёт хорошую оперативность, но зависит от добросовестности дочернего агента в сообщении о своём состоянии. На практике состояние дочернего агента часто моделируют как **конечный автомат** (отправлено, выполняется, требуется ввод, завершено, провалено); протокол A2A, о котором пойдёт речь далее в этой главе, как раз стандартизирует жизненный цикл задачи в виде таких состояний. Кроме того, в качестве подстраховки нужны **тайм-ауты и проверка активности («сердцебиение»)** (перекликается с Heartbeat и monitor_shell из четвёртой главы): даже если дочерний агент не сообщает о себе и не возвращает результат, главный агент может по правилу «нет активности дольше N минут — считать провалом» избежать зависания всей системы из-за застрявшего дочернего агента.

**Третье. Прерывание выполнения.** В параллельном взаимодействии часто возникает ситуация «один добился успеха, остальные уже не нужны» — несколько агентов ведут параллельный поиск, и как только один находит цель, остальные должны немедленно остановиться (каскадное завершение из эксперимента 10-6 этой главы). Прерывание бывает двух степеней жёсткости. **Мягкое прерывание (graceful)** — предпочтительный вариант: главный агент подаёт сигнал `terminate`, дочерний агент реагирует на него в безопасной точке текущего шага, сначала освобождает ресурсы (закрывает сессию браузера, дописывает незавершённый файл, снимает блокировку), возвращает подтверждение (ack) и только потом завершается. **Принудительное прерывание (forced)** — крайняя мера: процесс завершается напрямую, и применяется только тогда, когда дочерний агент не реагирует на мягкий сигнал; ценой становится риск оставить висящие ресурсы и незавершённые записи. Здесь важны два инженерных момента: во-первых, мягкое прерывание требует, чтобы дочерний агент периодически проверял сигнал завершения в своём цикле (аналогично механизму прерывания из четвёртой главы), иначе сигналу просто не на что будет откликнуться; во-вторых, при каскадном завершении возникает условие гонки — несколько дочерних агентов могут почти одновременно сообщить об успехе, и главный агент должен с помощью блокировки или идемпотентной схемы гарантировать, что фиксация результата произойдёт только один раз и сигнал завершения будет разослан только одним раундом; подробнее об этом условии гонки — в обсуждении эксперимента 10-6 этой главы.

Обмен артефактами (плоскость данных) вместе с передачей сообщений, запросом состояния и прерыванием выполнения (плоскость управления) образуют основу для многоагентных систем без общего контекста. Три топологии сотрудничества, о которых пойдёт речь ниже, по сути представляют собой разные варианты распределения контроля и направления потоков информации поверх этих двух плоскостей.
...
Абстракция «агенты как инструменты друг для друга» уже была установлена в разделе «Инструмент сотрудничества» четвёртой главы: дизайн интерфейсов spawn_subagent / send_message / cancel_subagent, а также четыре стратегии подготовки контекста для подчинённого агента (минимальная передача, ручной отбор, автоматическое усечение, генерация контекста с помощью LLM) — всё это напрямую применимо к рассматриваемому здесь вызову подчинённых агентов менеджером. Четвёртая глава решала вопрос о том, что передавать в направлении «менеджер → подчинённый агент»; симметричный вопрос — что возвращать в направлении «подчинённый агент → менеджер». Ответ: **структурированное резюме, а не полная траектория**. Подчинённый агент должен возвращать вывод по задаче, ключевые находки, пути к файлам-результатам и встреченные проблемы, оставляя полную траекторию выполнения в собственном логе. Только так контекст менеджера будет расти медленно и линейно с числом подзадач, а не взрывообразно — именно на этой методологической основе строится описанный ниже подход в эксперименте 10-3, где менеджер «поддерживает только индекс файлов, не сохраняя содержимое перевода». Разделение труда между главами таково: четвёртая глава рассказывает о механизме (реализация интерфейсов инструментов и передачи контекста), эта глава — об архитектуре (как строится топология и распределяются обязанности).
...
> **Эксперимент 10-3 ★★: агент перевода книги**
>
...
>
> **Эксперимент 10-5 ★★★: агент с автономной оркестрацией телефонных звонков и работы за компьютером**
>
> Архитектура сотрудничества двух агентов в эксперименте 10-4 была спроектирована заранее. Этот эксперимент идёт дальше и исследует **способность агента к автономной оркестрации** — сам агент решает, когда нужно запустить нового совместного агента, вместо того чтобы человек заранее продумывал весь процесс взаимодействия.
>
...
>
>
> **Эксперимент 10-6 ★★★: агент, одновременно собирающий информацию с нескольких сайтов**
>
...
>
> Этот эксперимент исследует применение параллельного выполнения несколькими агентами в сценарии сбора информации. В отличие от экспериментов 10-4 и 10-5, где рассматривается сотрудничество двух гетерогенных агентов, здесь речь идёт о **параллельном поиске несколькими однотипными агентами** и о том, как централизованная координация обеспечивает эффективное выполнение задачи и оптимизацию ресурсов.
>
...
### Децентрализованная модель: равноправная передача управления


![Рис. 10-10 Цепочная модель Handoff](images/fig10-10.svg)


Модель оркестрации, хотя и обеспечивает чёткую структуру управления и глобальный обзор, из-за своей централизованной природы несёт в себе неизбежные ограничения: менеджер становится узким местом системы и единой точкой отказа, все решения по координации зависят от его суждений, а сам менеджер должен при этом достаточно хорошо понимать все подзадачи. С ростом сложности задачи и увеличением числа агентов масштабируемость такой архитектуры оказывается под угрозой.

Децентрализованная модель предлагает другой архитектурный подход: **нет единого центрального управляющего, агенты взаимодействуют друг с другом на равных**. Каждый агент, исходя из собственного профессионального суждения, самостоятельно решает, когда обратиться к другому агенту — это может быть передача задачи («моя часть готова, дальше твоя очередь»), запрос на обратную связь («это решение технически осуществимо?») или сообщение о проблеме («в переданных тобой требованиях есть противоречие, нужно обсудить заново»).

Три приведённых ниже примера намеренно выстроены в линию «от псевдо- к настоящей» децентрализации: управляющий поток MetaGPT на деле представляет собой жёсткий конвейер (псевдодецентрализация, развязка происходит только на уровне механизма коммуникации), group chat в AutoGen — это гибридная форма из общей истории диалога и централизованного диспетчера, и лишь в OpenAI Swarm достигается по-настоящему равноправная децентрализация именно на уровне управляющего потока.

**Что передаётся при handoff без общего контекста?** Цепочная модель Handoff на рис. 10-10 напрямую контрастирует с `transfer_to_agent` из эксперимента 10-2: во втором случае передача происходит при общем контексте, и новая роль автоматически наследует полную историю без какого-либо специального проектирования; в первом же случае передача происходит без общего контекста, и передающая сторона должна явно решить, что именно передавать. На практике эффективный «пакет передачи» обычно состоит из трёх частей: **описание задачи** (что должен сделать получатель и каковы критерии приёмки), **подтверждённые факты и ограничения** (предпочтения пользователя, бизнес-правила, решения, зафиксированные на предыдущих этапах) и **ссылки на структурированные артефакты** (пути к файлам, а не их содержимое — получатель читает их по мере необходимости). Намеренно не передаётся полная траектория — процесс проб и ошибок передающей стороны, промежуточные рассуждения и неудачные попытки для получателя в основном представляют собой шум. В этом и заключается принципиальное различие между двумя видами передачи: передача с общим контекстом сохраняет полную историю — информация не теряется, но контекст постоянно разрастается; передача без общего контекста передаёт очищенный пакет — информация теряется частично, зато каждый агент работает в чистом, сфокусированном контексте. Каждому агенту не нужно понимать «ход мыслей» другого агента — достаточно понимать формат и семантику пакета передачи и произведённых артефактов. Такое сотрудничество на основе интерфейсов заимствует принцип контрактного проектирования из инженерии программного обеспечения.

**MetaGPT: симуляция софтверной компании на основе SOP (переходный пример от конвейера к развязанной коммуникации).**


![Рис. 10-11 Сеть мультиагентного взаимодействия MetaGPT](images/fig10-11.svg)


Ключевая идея MetaGPT состоит в том, что накопленные в человеческих софтверных компаниях **стандартные операционные процедуры** (SOP, Standard Operating Procedure) сами по себе являются многократно проверенным протоколом взаимодействия — если закодировать SOP в многоагентную систему, каждая роль будет производить стандартизированные результаты, как специализированный участок конвейера, а сами результаты естественным образом становятся интерфейсом коммуникации между ролями.

В MetaGPT роли работают в фиксированном порядке (агент менеджера продукта → агент архитектора → агент менеджера проекта → агенты инженеров → агент QA-инженера), каждая роль выдаёт структурированный результат:

- **Агент менеджера продукта**: получает описание требований, генерирует структурированный PRD (документ с требованиями к продукту, включающий список функций, пользовательские истории, критерии приёмки, приоритизацию)
- **Агент архитектора**: читает PRD, принимает архитектурные решения (выбор технологического стека, разбиение на модули, определение интерфейсов, проектирование модели данных), выдаёт проектный документ
- **Агент менеджера проекта**: читает проектный документ, разбивает систему на конкретный список задач и распределение по файлам, выстраивает порядок зависимостей между модулями, затем распределяет задачи между инженерами
- **Агенты инженеров**: читают проектный документ, реализуют закреплённые за ними модули, производят код. Могут работать параллельно в нескольких экземплярах
- **Агент QA-инженера**: читает код и PRD, генерирует тестовые сценарии, выполняет тесты, фиксирует баги, выдаёт отчёт о тестировании

Настоящий вклад MetaGPT в децентрализованную коммуникацию заключается в механизме передачи информации: **общий пул сообщений + подписка по ролям**. Каждая роль публикует структурированные сообщения в пуле, видимом всем ролям, а остальные роли забирают из него только те сообщения, которые связаны с их зоной ответственности, согласно собственной настройке подписки — вместо точечной передачи сообщений один к одному. Публикующей стороне не нужно знать, кто будет потреблять её вывод, а для добавления новой роли достаточно объявить, на какие типы сообщений она подписывается, без изменения уже существующих ролей. Это обеспечивает настоящую развязку: например, если заменить агента менеджера продукта на более сильную модель, при условии что публикуемый ею PRD по-прежнему соответствует спецификации, все остальные агенты не потребуют никаких изменений.

Итеративное улучшение в MetaGPT происходит главным образом на этапе работы инженера, и механизм здесь — **исполняемая обратная связь** (executable feedback): инженер запускает написанный им код и тесты, по сообщениям об ошибках и результатам неудачных прогонов входит в цикл отладки, пока тесты не пройдут — исправление направляется детерминированным результатом выполнения, а не мнением другого агента.

Честности ради стоит сказать: MetaGPT не является децентрализованной системой **на уровне управляющего потока** — порядок ролей заранее зафиксирован SOP, и в целом это скорее конвейер (на языке первой главы — рабочий процесс). Он рассматривается в этом разделе потому, что механизм коммуникации на основе пула сообщений с подпиской демонстрирует важнейший элемент дизайна децентрализованных систем: развязку. Что же касается таких сценариев, как «QA напрямую обращается к менеджеру продукта за уточнением требований» или «инженер обсуждает с архитектором альтернативные варианты», — это лишь естественное расширение данной архитектуры в качестве возможного замысла; в оригинальной версии MetaGPT оно не реализовано.

**AutoGen group chat: общая история диалога + централизованная диспетчеризация.** В AutoGen group chat позволяет нескольким агентам участвовать в одном и том же разговоре: на каждом ходу «селектор выступающих» решает, какой агент говорит следующим — селектором может быть простое правило ротации или LLM, определяющая, кому лучше всего ответить исходя из текущего содержания диалога; реплика любого агента видна всем участникам. Честности ради нужно отметить, что это не система, полностью децентрализованная в смысле управляющего потока: выбор выступающего единолично производит централизованный GroupChatManager, а само решение «чья очередь говорить» — это и есть решение по управляющему потоку. Поэтому более точное определение этой модели — **гибридная форма «общая история диалога + централизованная диспетчеризация»**: все агенты видят одну и ту же общую запись диалога, но каждый сохраняет собственный системный промпт и набор инструментов, а полномочия по диспетчеризации сосредоточены в руках селектора. Такая модель подходит для задач, требующих обсуждения с разных точек зрения, где порядок высказываний трудно зафиксировать заранее (например, рецензирование решений, межпредметный анализ), ценой чего становится риск расхождения диалога, требующий тщательного проектирования условий завершения. По принятому в этой главе разделению её отнесли сюда исходя из механизма диспетчеризации (централизованный селектор), но по измерению общего контекста она находится где-то между общим и раздельным контекстом, представляя собой гибридную форму — это ещё раз показывает, что топология и совместное использование контекста концептуально независимы друг от друга и могут комбинироваться по-разному.

**OpenAI Swarm и Agents SDK: сеть handoff.** В противовес этому, настоящим представителем равноправной децентрализации именно на уровне управляющего потока является Swarm от OpenAI (и его преемник Agents SDK): здесь децентрализация реализована в предельно простой форме — каждый агент снабжён несколькими вариантами handoff (передачи), позволяющими в любой момент передать управление любому другому агенту в сети. Агент сортировки обращений в поддержку, определив, что вопрос касается возврата средств, передаёт управление агенту по возвратам; агент по возвратам, обнаружив в процессе, что дело в технической неисправности, может, в свою очередь, передать управление агенту технической поддержки. В системе нет центрального диспетчера, управление передаётся эстафетой между равноправными агентами, а решения о маршрутизации полностью распределены — каждый агент принимает их самостоятельно. Это и есть чистая «равноправная передача управления», и именно она служит инженерной реализацией цепочной модели передачи, показанной на рис. 10-10.

### Межорганизационное взаимодействие: протокол A2A

Все описанные выше системы предполагают, что все агенты разработаны одной командой и работают в рамках одной системы — в этом случае трёх механизмов коммуникации (передача параметров, общие файлы, шина сообщений) достаточно. Но когда взаимодействие выходит за границы организации — например, вашему агенту нужно вызвать агента другой компании, — требуется стандартизированный протокол взаимодействия. Именно для этого в 2025 году Google выпустил протокол **A2A** (Agent2Agent) (позже переданный на попечение Linux Foundation). У него три ключевых элемента:
...
Общая файловая система — ключевая инфраструктура мультиагентного взаимодействия, но когда несколько агентов работают с ней одновременно, конкурентные конфликты становятся неизбежным инженерным вызовом. Эти конфликты делятся на два вида.
...
Нужно отметить: рефлексия здесь отличается от рефлексии в самоэволюции агента из восьмой главы: рефлексия из восьмой главы происходит **после завершения задачи** и предназначена для обновления долгосрочных способностей; рефлексия здесь происходит **в ходе повседневной деятельности генеративного агента** и предназначена для обновления сиюминутного внутреннего состояния и целей.
...
> **Эксперимент 10-7 ★: Запуск Stanford AI Town**
>
> **Шаги эксперимента**:
...
> **Эксперимент 10-8 ★★★: Голосовая система агентов для игры в Мафию**
>
> Мафия — классическая социально-дедуктивная игра, проверяющая у игроков способность к рассуждению, навыки обмана и социальную стратегию. В этом эксперименте нужно построить многоагентную систему, где ИИ-агенты играют различные роли в Мафии вместе с живыми игроками через голосовое взаимодействие в реальном времени — это одновременно проверяет способности агента к рассуждению, ролевой игре и интерактивности в реальном времени.
>
> **Архитектура решения**:
>
> **1. Управление состоянием игры**: судья (управляемый кодом, не LLM) поддерживает централизованное состояние — список игроков (смесь живых людей и ИИ), их роли, принадлежность к лагерю, состояние выживания, фазу игры (ночь/день/голосование/подведение итогов), журнал исторических событий.
>
...
>
> **3. Голосовое взаимодействие в реальном времени**: эксперимент требует использования голосовых возможностей в реальном времени для реализации голосовой связи между живыми игроками и ИИ-агентами. Рекомендуется взять за основу голосового агента реального времени из главы 9. На дневном этапе обсуждения судья управляет порядком выступлений — можно давать слово по кругу или разрешать поднимать руку для запроса слова. На этапе голосования собираются голоса всех игроков (живые люди выражают их голосом, ИИ принимает решение через рассуждение), после подсчёта голосов объявляется изгнанный игрок.
>
> **4. Рассуждение и стратегия агента**:
>
...
>
> **Критерии приёмки**:
> - Настроена партия на 6–8 человек (1 живой игрок + 5–7 ИИ-агентов)
> - Распределение ролей: 2 оборотня, 1 провидец, 1 ведьма, остальные — жители; живой игрок получает роль случайным образом
...
У многоагентных систем есть два ортогональных ключевых измерения проектирования: общий ли контекст между агентами и как организована топология сотрудничества. Общий контекст — это «наследующее» мультиагентное взаимодействие: последующий агент наследует полный контекст предшествующего, информация не теряется, но контекст быстро разрастается; отсутствие общего контекста означает полностью независимое мультиагентное взаимодействие, где информация передаётся через сжатые пакеты передачи, файловую систему или обмен сообщениями. Что касается топологии сотрудничества, одноранговый режим подходит для итеративного улучшения небольшого числа агентов, модель оркестрации — для сложных задач, требующих динамического планирования, а децентрализованная модель — для сценариев с равноправными обязанностями, где контроль должен свободно перетекать между агентами. Всё это опирается на два вида инфраструктуры, не зависящей от топологии: **общая файловая система** как плоскость данных — по сути виртуальное дерево каталогов, смонтированное из четырёх типов областей: собственных рабочих пространств агентов, общего пространства нескольких агентов, внешних ресурсов и встроенных системных ресурсов, — где агенты обмениваются продуктами работы через передачу путей к файлам; **механизм коммуникации и управления** как плоскость контроля поддерживает передачу сообщений, запрос состояния и завершение выполнения. Шина сообщений — распространённая реализация плоскости контроля, подходящая для координации сообщений в реальном времени, асинхронно и между многими сторонами; при пересечении границ организаций нужен стандартизированный протокол взаимодействия вроде A2A.

Исследования последних лет выявили ключевой критерий для определения того, превосходит ли многоагентная система одноагентную: **вносит ли процесс сотрудничества новую информацию, которой не было на этапе генерации**. Если несколько агентов просто повторно рассматривают один и тот же текст (как в режиме дебатов), то при равном объёме вычислительных ресурсов одиночный агент столь же эффективен; но если Reviewer может получить внешнюю обратную связь — результаты выполнения кода, скриншоты визуального рендеринга, вывод верификации инструментов, — преимущество многоагентной системы становится существенным. Именно в этом смысл тезиса Loop-инженерии «узкое место цикла — верификатор»: чтобы покончить с тремя видами преждевременного завершения — ленивым ложным завершением, преждевременным отказом и ложным успехом, — момент завершения задачи должен определять верификатор, опирающийся на реальные наблюдения, а не заявление самой модели. Кроме того, увеличение бюджета шагов для агента само по себе не приводит автоматически к лучшим результатам — нужен явный механизм бюджетно-ориентированного поведения, направляющий агента к разумному распределению вычислительных ресурсов. В модели оркестрации возможности планировщика — узкое место всей системы, поэтому самую сильную модель и наиболее тщательно проработанный промпт нужно выделять именно агенту, отвечающему за планирование.

Когда число агентов становится достаточно большим, у них возникает коллективное поведение, которое невозможно спроектировать заранее. 25 агентов Стэнфордского AI города спонтанно распространяли сообщения и координировали организацию встреч; 1,5 миллиона агентов на Moltbook породили цифровую религию и машинно-нативные протоколы сотрудничества. В экономическом измерении конкурирующие друг с другом агенты в Vending-Bench Arena вступили в ценовые войны и даже самопроизвольно сговорились о ценах, Pinchwork позволяет агентам нанимать друг друга через рыночный механизм, а RentAHuman даёт агентам возможность нанимать людей за криптовалюту для выполнения физических задач. Это намекает на новое направление координации — децентрализованное распределение ресурсов на основе рыночного механизма. Насколько оно похоже или отличается от трёх обсуждённых ранее архитектур — вопрос, заслуживающий дальнейшего изучения.
