...
持续写入能力会把一次性错误变成长期风险。网页、邮件和工具输出中的提示注入若被总结成经验，可能跨会话反复生效；自动搜索的恶意软件包若被封装成工具，影响会从一次沙盒运行扩散到所有后续任务；一个有缺陷的验证器还可能持续批准看似进步、实际退化的候选版本。因此，进化系统除了验证“是否更强”，还必须限制“谁能改什么、依据来自哪里”。

第一道边界是**证据与指令隔离**。原始网页和工具输出是不可信证据，写入前要保留来源、时间、环境版本和信任级别，并扫描提示注入、凭证外泄和不可见字符等模式。检索出的经验默认以参考资料身份进入上下文，不自动拥有系统指令权限；只有经过多轨迹验证和审批的规则才能提升为 Prompt 或 Skill。用户纠正也应注明作用域，避免把个人偏好升级为所有用户的全局政策。

第二道边界是**候选能力与正式能力隔离**。新知识、Prompt、Skill、程序和参数都先进入不可服务真实流量的候选区；代码和外部依赖还要经过沙盒、权限检查、供应链扫描与行为测试。发布系统只接受结构化 manifest，其中包含来源、diff、评估结果、版本和回滚点。无法重置的高风险环境不能用真实转账、下单或发信来自动验证候选。

第三道边界是**可信根不可自改**。业务 Agent 可以提出修改工具、路由和规则，但不能同时修改批准自身更新的验证器、发布门槛、审计日志、权限策略和稳定版本备份。高风险系统还应让生成候选、执行测试和批准发布由不同身份完成。否则，一个 Agent 只需降低测试阈值或删除失败用例，就能把退化伪装成进步。
...
在线执行与离线“睡眠学习”必须保持权限分离：前者不能依据单次经历直接改写正式能力，后者也不能修改批准自身更新的可信根。只有来源可追溯、候选与正式版本隔离、验证门槛独立且能够回滚，经验积累才不会演变为长期的错误放大。
