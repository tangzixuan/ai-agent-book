...
本 demo 为控成本采用 **7 人局**（全部由 AI 扮演，保证可复现可验证）：
**2 狼人 + 1 预言家 + 1 女巫 + 3 村民**。
...
│   ├── agent.py         # PlayerAgent：每玩家一个 LLM Agent + 私有上下文 + LLM 调用
...
- **模型只用 OpenAI**（`OPENAI_API_KEY`，默认 `gpt-4o-mini`）。按实验约束，
